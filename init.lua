...
-- Selection overwrite paste
vim.keymap.set('n', '<leader>p', '"_dP', { desc = 'Overwrite selection with paste' })
...
require('lazy').setup({
  ...
  --
  -- Upstream replaced vim-sleuth with guess-indent.nvim (faster, actively
  -- maintained). Taking the swap; the two do the same job.
  ...
  -- Fuzzy Finder (files, lsp, etc)
  {
    ...
    config = function()
      ...
      -- NOTE: personalized to the <leader>f* ("find") prefix instead of
      -- upstream's <leader>s* ("search"). Descriptions keep upstream's wording.
  ...
  -- Used to highlight, edit, and navigate code
  {
    ...
    --     no longer exist).
    config = function()
      -- Ensure basic parsers are installed (personalized: adds json, python,
      -- toml and yaml on top of upstream's base list).
      local parsers = {
        'bash',
        'c',
        'diff',
